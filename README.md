...
### Chạy trên iPhone
- Cài đặt app **Expo Go** từ App Store trên iPhone.
- Đảm bảo iPhone và máy tính đang kết nối cùng một mạng Wi-Fi.
- Chạy lệnh:
   ```bash
   npx expo start
   ```
- Nếu Expo hiển thị QR code trong trình duyệt, dùng Expo Go trên iPhone quét QR code.
- Nếu không quét được, chọn chế độ **Tunnel** trong Expo DevTools rồi quét lại.
- App sẽ khởi chạy trên iPhone trong Expo Go và tự động reload khi bạn thay đổi code.
